...
Edituj `filtrovna_behavior/config.json` před spuštěním:
...
Unit testy čistých funkcí (itemPriority, checkFilter, inferTags):
...
Zapni `debug_mode: true` v config.json pro detailní logy v konzoli.
...
- **Batch modes:** `group_by_type` (legacy, jen jeden typ) nebo `fill_any` (top N dle priority, mix typů).
...
- **Funkce:** 
  - 5 filtr slotů (pro whitelist specifických itemů) + 27 buffer pro storage
  - Prioritizuje vzácné/filtrované předměty (diamanty > iron > stone)
  - Zadrží předměty při plném výstupu
  - **Strict filter mode:** ignoruje nefiltrované itemy
  - **Priority mode:** zpracuje filtrované dříve, ale i nefiltrované (nižší priorita)
  - Konfigurovatelný počet pickupů za tick (`max_pickups_per_tick`)
  - Podpora pro chest/barrel/hopper/shulker/ender chest jako cíl
- **Konfigurace:**
  ```json
  {
    "smart_hopper": {
...
**Testované funkce:**
- `itemPriority(typeId)` — ověří že ore < ingot, diamond > iron atd.
- `checkFilter(item, filterIds, mode)` — testuje exact/mod/tag režimy
- `inferTags(typeId)` — ověří tagování (ore, ingot, tool, armor atd.)

### Manuální scénáře v Minecraft

1. **Smart Hopper strict mode:**
   - Nastav `strict_filter: true`
   - Vrž diamant a hlínu
   - Ověř že jen diamant je sebrán

2. **Smart Hopper priority mode:**
   - Nastav `strict_filter: false`
   - Vrž mix itemů
   - Ověř že vzácnější jsou dříve

3. **Filtr batch modes:**
   - `batch_mode: "group_by_type"` — batch všechny železné ingoty
   - `batch_mode: "fill_any"` — batch top 3 itemy bez ohledu na typ
...
filtrovna_behavior/     # Behavior Pack
  ...
  scripts/                # Script API (main.js + modules/)
    modules/
      ├── config.js       # Konfigurace + validace
      ├── logger.js       # Centralizované logování
      ├── transfer_logic.js
      ├── smart_hopper.js
      ├── tick_handler.js
      ├── inventory_manager.js
      ├── test_suite.js   # POC testy
      └── [dalších modulů...]
...
**Poslední update:** 2026-08-19 — Opravy v5 (logger, config validation, test suite, Smart Hopper strict/priority)
